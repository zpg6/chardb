...
// A generated dev process that hits an occupied port still has to start Node,
// load Wrangler, and fail to bind before it can exit. Cold hosted Windows
// runners have needed well over ten seconds for that path.
const STARTUP_FAILURE_TIMEOUT_MS = 60_000;
// Descendant cleanup is verified through Win32_Process snapshots that take
// about a second each on hosted runners, and the generated watchdog needs
// several of its own snapshots to finish killing orphans. Five seconds was
// only enough on a quiet runner.
...
try {
    ...
    for (let iteration = 0; iteration < 3; iteration++) {
        ...
        try {
            ...
            await runPhase(`restart cycle ${cycle}: verify descendant cleanup`, () =>
                waitForProcessesToExit(descendants, PROCESS_EXIT_TIMEOUT_MS)
            );
            await runPhase(`restart cycle ${cycle}: verify project cleanup`, () =>
                waitForNoProjectProcesses(project, PROCESS_EXIT_TIMEOUT_MS)
            );
    ...
    async function proveOccupiedPortFailure({ blockedServer, blockedPort, workerPort, webPort, service }) {
        ...
        await waitForNoProjectProcesses(project, PROCESS_EXIT_TIMEOUT_MS);
...
async function waitForProcessesToExit(processes, timeoutMs) {
    ...
    if (remaining.length > 0) {
        throw new Error(
            `dev descendants survived shutdown after ${timeoutMs}ms: ${remaining
                .map(process => `${process.pid} ${process.commandLine.slice(0, 120)}`)
                .join("; ")}`
        );
    }
}
